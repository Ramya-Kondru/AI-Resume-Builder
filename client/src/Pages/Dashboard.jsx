// user can upload or create the resume and the list of resumes user has already created
// FilePenLineIcon-resume Card
// PencilIcon-edit
// TrashIcon-delete
...
// useNavigate-page navigation inside React app.
...
const Dashboard = () => {
  ...
  // Later, each resume card uses one color from this list to look different.
  const colors = ['#9333ea', '#d97706', '#dc2626', '#0284c7', '#16a34a']
  // to add dummy resume data from assets.js file
  // all resume cards shown on dashboard
  const [allResumes, setAllResumes] = useState([])

  // controls whether create resume pop is visible
  const [showCreateResume, setshowCreateResume] = useState(false)

  const [showUploadResume, setshowUploadResume] = useState(false)

  // store text entered in input box
...
toast.error(error?.response?.data?.message || error.message)
  ...
  setIsLoading(false)
    ...
    // execute the function whenever the function gets loaded,when page loads all resumes appear
    useEffect(() => {
      loadAllResumes()
    }, [])
    return (
      <div>
        ...
        to-slate-700 bg-clip-text text-transparent sm:hidden'>Welcome, Ramya</p>
        ...
        gap-2 text-slate-600 border border-dashed border-slate-300 group hover:border-indigo-500
        ...
        gap-2 text-slate-600 border border-dashed border-slate-300 group hover:border-purple-500
        hover:shadow-lg transition-all duration-300 cursor-pointer'>
          ...
          <div className='grid grid-cols-2 sm:flex flex-wrap gap-4'>
            {allResumes.map((resume, index) => {
              ...
              return (
                // by clicking on any resumes it will redirect to resume builder page 
          ...
          {showCreateResume && (
            ...
            w-full max-w-sm p-6'>
                ...
                <input onChange={(e) => setTitle(e.target.value)} value={title}
                  type='text' placeholder='Enter resume titile' className='w-full px-4 py-2 mb-4 focus:border-green-600 ring-green-600' required />
  ...
  <form onSubmit={uploadResume} onClick={() => setshowUploadResume(false)} className='fixed inset-0 bg-black/70 backdrop-blur bg-opacity-50 z-10 flex items-center justify-center'>
    ...
    w-full max-w-sm p-6'>
      ...
      <input
        ...
        value={title}
        type='text'
        placeholder='Enter resume titile'
